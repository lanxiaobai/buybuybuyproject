    ...
    return {
      ...
      rules: {
        accept_name: [
          ...
          { min: 2, message: "长度在 2 到 10 个字符", trigger: "change" }
        ],
        address: [
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            .then(response=>{
                ...
                // 订单创建成功之后,修改同步右上角购物车的数量
                // 删除对应的id
                // 因为之前的id是用逗号间隔,所以要截取字符串
                let idArr=this.orderInfo.goodsids.split(',');
                // 循环遍历idArr这个数组,然后 通知Vuex删除了对应的数据
                idArr.forEach(v=>{
            ...
            this.$Message.error('chack it agin!!!');
            return false;
          }
        });
    }
